...
def run_mcp_server(main_agent: CodeActAgent, mcp_config: Dict[str, Any]) -> None:
    ...
    for agent in main_agent.get_all_agents():
        ...
        async def agent_tool(  # type: ignore
            query: str, session_id: Optional[str] = None, agent_instance=agent
        ) -> str:
            ...
            async def run_agent(query: str) -> str:
                ...
                response: str = await agent_instance.ainvoke(
        ...
        proxy.add_tool(
            proxy.tool(name="agent__" + agent.name, description=agent.description)(agent_tool)
        )
    ...
    async def agent_logs(session_id: str) -> str:
        ...
        while not queue.empty():
            ...
            elements.append(event.data["text"])
